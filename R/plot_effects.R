...
plot_effects <- function(sgb_model, plot_type = 'radar', prop = 1, n_vars = 30,
  ...
  ggplot2::ggplot(aes(x = .data$x, y = .data$y, xend = .data$x*0.13, yend = .data$y*0.13)) +
    ...
    plotdata <- plotdata %>%
      dplyr::mutate(x = abs(.data$effect)*cos(-.data$cum_importance*1.5*pi),
    ...
    plot_out <- plotdata %>%
      ...
      ggforce::geom_circle(aes(x0 = 0,y0=0, r = max_lim*1.15*0.5), linetype = 3,
                           color = 'lightgray') +
      ggplot2::geom_segment(aes(x = 0, y = 0, xend = 0,yend = max_lim*1.15), data = data.frame(),
                            linetype = 2, color = 'grey') +
      ggplot2::geom_curve(aes(x = .data$x, y = .data$y, xend = max_lim*1.15*cos(pi*0.45), yend = max_lim*1.15*sin(pi*0.45)),
                          data = data.frame(x = 0,y = max_lim*1.15, xend = max_lim*1.15, yend = 0.25),
                          arrow = arrow(length = unit(6, 'pt')), angle = 0, color = 'grey') +
      ggplot2::geom_label(aes(color = .data$type, label = .data$variable),
                          fontface = "bold", alpha = 0.9) +
      ggplot2::geom_label(aes(x = .data$x/2, y = .data$y/2,label = round(.data$effect,2)), size = 2, alpha = 0.9) +
    ...
    plot_out <- plotdata %>%
      ...
      ggplot2::theme_classic() +
      ggplot2::theme(legend.title=element_blank()) +
      ggplot2::geom_curve(aes(x = .data$cum_importance, y = 0, xend = .data$cum_importance,
                              yend = .data$effect, color = .data$type),angle = 0) +
      ggplot2::geom_label(aes(x = .data$cum_importance, y = .data$effect/2,label = round(.data$effect,2)), size = 2, alpha = 0.9) +
      ggplot2::geom_label(aes(color = .data$type, label = .data$variable),
                          fontface = "bold", alpha = 0.9) + ggplot2::xlim(c(0,1)) +
      ggplot2::xlab('Cumulative relative importance') + ggplot2::geom_hline(yintercept = 0, linetype = 'dashed') +
