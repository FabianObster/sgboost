...
```{r, warning=FALSE,message=FALSE, eval=eval}
#library(sgboost)
# 10; 305, 588
set.seed(10)
n <- 100
...
reflect that final model estimates of a specific variable in the dataset can be attributed
to group baselearners as well as individual baseleraners depending on the boosting iteration

### Variable importance
A good starting point for understanding a sparse-group boosting model is the 
variable importance. Generally the variable importance can be defined as the 
relative contriubtion of each predictor to the overall reduction of the loss function
(negative log likelihood). This can be done for each baselearner differentiating 
between group and individual variable or aggregated for each variable in the dataset.
`get_varimp` returns both in a data.frame. As well as the total importance of all
groups vs. individual variables. In `plot_varimp` one can indicate the maximum 
number of predictors to be printed through `n_predictors` or the maximal 
variable importance that the most important predictors combined can achieve `prop`.
Through both parameters the number of printed entries can be reduced. Note that
in this case the relative importance of groups in the legend is based only on the
plotted variables and not the ones removed.
```{r, eval=eval, fig.align='center', fig.height=5, fig.width=7}
get_varimp(sgb_model = sgb_model_binary)
plot_varimp(sgb_model = sgb_model_binary, n_predictors = 10) + theme_bw(base_size = 8)
```
...
for the same variable. This function aggregates both and returns it in a data.frame
...
With `plot_effects` we can plot the effect sizes of the sparse-group boosting model in relation to 
the relative importance to get an overall picture of the model. 
Throug the parameter `'plot_type'` one can choose the type of visualization.
...
```{r, eval=eval, fig.align='center', fig.height=5, fig.width=7}
plot_effects(sgb_model = sgb_model_binary, n_predictors = 10) + theme_bw(base_size = 8)
plot_effects(sgb_model = sgb_model_binary, n_predictors = 10, plot_type = 'clock') + theme_bw(base_size = 8)
plot_effects(sgb_model = sgb_model_binary, n_predictors = 10, plot_type = 'scatter') + theme_bw(base_size = 8)
```
...
```{r, eval=eval, fig.align='center', fig.height=5, fig.width=7}
plot_path(sgb_model = sgb_model_linear[100]) + theme_bw(base_size = 8)
plot_path(sgb_model = sgb_model_binary[100])+ theme_bw(base_size = 8)
