...
`cvrisk` can be used und plotted. `cvrisk` may be slow to run. One can run it in
...
defined as the relative contriubtion of each predictor to the overall reduction
...
plot_varimp(sgb_model = sgb_model_binary, n_predictors = 10) 
```
...
```{r, eval=eval, fig.align='center', fig.height=5, fig.width=7}
plot_effects(sgb_model = sgb_model_binary, n_predictors = 10) 
plot_effects(sgb_model = sgb_model_binary, n_predictors = 10, plot_type = 'clock') 
plot_effects(sgb_model = sgb_model_binary, n_predictors = 10, plot_type = 'scatter')
```
