...
sgb_model_binary <- sbb_model_binary{540}
get_coef(sgb_model = sgb_model_linear)
get_coef(sgb_model = sgb_model_binary)
```
...
for the same variable. This function aggregates both and returns it in a dataframe.

```{r, eval=eval}
get_coef(sgb_model = sgb_model_linear)$aggregate %>% slice(1:10)
get_coef(sgb_model = sgb_model_linear)$raw %>% slice(1:10)
```
#### `get_varimp` and `plot_varimp`
```{r, eval=eval}
